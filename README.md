...
* **Layout**
  ...
  * `SET_COLOR`
	...
	* (argument 1) is the new color in the format `r,g,b` with each value ranging from 0 to 255 or a color code.
	* For color codes, see https://github.com/FMMT666/launchpad.py#color-codes, however not all color codes have been implemented yet.
	...
	* If the mode of the button has been changed before via `SET_COLOR_MODE`, it is reset to `solid`.
  * `SET_COLOR_MODE`
    * Changes the color and the mode of a single button
	* (argument 1) is the new color as color code. RGB-values are not supported!
	* (argument 2) is the new mode of the button. Supported values are `solid`, `flash` and `pulse`.
	* For color codes, see https://github.com/FMMT666/launchpad.py#color-codes, however not all color codes have been implemented yet.
	* Optionally, (argument 3) and (argument 4) may specify the button in x- and y-coordinates ranging from 0 to 8 each. If not supplied, the color and mode of the pressed button is changed.
